...
import { getSliderPhotos } from "@/lib/data"
...
export function HomepageSlideshow() {
  ...
  React.useEffect(() => {
    ...
    api.on("select", () => {
       // Acknowledge interaction, but don't stop autoplay. Or could restart it here.
    })
    ...
    <section className="w-full relative">
      <Carousel setApi={setApi} className="w-full" opts={{ loop: true }}>
       ...
       <div className="absolute bottom-10 text-center w-full z-10">
            ...
            <p className="text-white/90 text-lg md:text-xl mt-2 drop-shadow-md">Your community, your space.</p>
        </div>
    </section>
  )
}
