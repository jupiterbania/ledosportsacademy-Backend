...
export default function GalleryManagementPage() {
  ...
  const onSubmit = async (data: PhotoFormValues) => {
    const { id, sendNotification, aiTopic, ...photoData } = data;
    try {
      if (id) {
  ...
  const handleEdit = (photo: Photo) => {
    form.reset({ ...photo, sendNotification: false, aiTopic: '' });
  ...
  const handleAiAction = async (action: 'enhance' | 'generate', fieldToUpdate?: 'title' | 'description') => {
    ...
    try {
        const input = action === 'generate' 
            ? { topic, context: 'gallery' as const } 
            : { title: fieldToUpdate === 'title' ? title : '', description: fieldToUpdate === 'description' ? description : '', context: 'gallery' as const };
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="bg-muted/50 p-4 rounded-lg border">
                                ...
                                <FormControl>
                                    <Textarea placeholder="Describe the photo in a few words (e.g., 'team celebrating championship win') and the AI will write the title and description for you." {...field} />
                                </FormControl>
